"""
SDF and quaternion operations for SECAD-Net.
"""
...
def transform_points(quaternion, translation, points):
    """Transform points using quaternion rotation and translation."""
...
def sdfBox(quaternion, translation, dims, points):
    """Compute signed distance field for a box primitive."""
    B, N, _ = points.shape
...
def sdfExtrusion(sdf_2d, h, points):
    """Compute signed distance field for extruded 2D profiles."""
    ...
    b = torch.cat(
        (
            d.max(torch.zeros_like(z_diff)).unsqueeze(-1),
            z_diff.max(torch.zeros_like(z_diff)).unsqueeze(-1),
        ),
        -1,
    )
    b = b.norm(dim=-1)
...
def add_latent(points, latent_codes):
    """Concatenate latent codes to point coordinates."""
